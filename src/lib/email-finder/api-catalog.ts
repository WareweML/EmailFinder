...
export const API_ENDPOINTS: ApiEndpoint[] = [
  ...
  },
  {
    ...
    summary: "ROAS-weighted audience from paying customer emails — person + company enrich, not job-board personas.",
    description:
      "Pass paying customer emails (highest ACV first). We person-enrich + company-enrich each buyer, then create a SparkToro v3 audience from those people and pull every section: demographics, networks, youtube, podcasts, websites, press, apps, social, reddit, prompts, bios, tam, keywords. Repeats of the same prompt are free. Requires SPARKTORO_API_KEY.",
    params: [
      { name: "website", in: "body", required: true, type: "string", description: "Your domain", example: "emailverifier.io" },
      { name: "brief", in: "body", type: "string", description: "What you sell" },
      { name: "customers", in: "body", type: "object[]", description: "{ email, acv?, name?, domain? } — email preferred" },
      { name: "competitors", in: "body", type: "string[]", description: "Competitor domains" },
    ],
    response: [
      { field: "buyers[]", type: "object[]", description: "Enriched paying people: name, title, company, industry, LinkedIn, ACV %" },
      { field: "segments[]", type: "object[]", description: "ACV-weighted clusters from those buyers" },
      { field: "demographics", type: "object", description: "titles, functions, seniority, industries, sizes, locations of people who paid" },
      { field: "lookalikes[]", type: "object[]", description: "Similar companies of paying logos" },
      ...
      { field: "takeAction[]", type: "string[]", description: "Where to show up for ROAS" },
    ],
    curl: `curl -s -X POST "https://{host}/api/v2/icp/find" -H "content-type: application/json" -d '{"website":"emailverifier.io","brief":"Email verification SaaS","customers":[{"email":"ayrton@curaeducation.com","acv":120000},{"email":"baki@lookfor.ai","acv":80000}],"competitors":["zerobounce.net"]}'`,
  },
  {
    id: "catalog",
    group: "Meta",
